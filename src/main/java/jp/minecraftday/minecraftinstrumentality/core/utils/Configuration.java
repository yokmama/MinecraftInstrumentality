package jp.minecraftday.minecraftinstrumentality.core.utils;
...
public class Configuration extends YamlConfiguration {
    ...
    public synchronized void load() {
        ...
        if (!configFile.getParentFile().exists()) {
            if (!configFile.getParentFile().mkdirs()) {
                LOGGER.log(Level.SEVERE, I18n.tl("error.config.failedToCreateConfig", configFile.toString()));
            }
        }
        ...
        if (!configFile.exists()) {
            if (legacyFileExists()) {
                ...
                LOGGER.log(Level.INFO, I18n.tl("message.config.creatingConfigFromTemplate", configFile.toString()));
                createFromTemplate();
            } else {
                return;
            }
    ...
    private void createFromTemplate() {
        ...
        try {
            ...
            if (istr == null) {
                LOGGER.log(Level.SEVERE, I18n.tl("error.config.couldNotFindTemplate", templateName));
                return;
            }
            ...
            LOGGER.log(Level.SEVERE, I18n.tl("error.config.failedToWriteConfig", configFile.toString()), ex);
        } finally {
            try {
                if (istr != null) {
                    istr.close();
            ...
            try {
                ...
                LOGGER.log(Level.SEVERE, I18n.tl("error.config.failedToCloseConfig", configFile.toString()), ex);
            }
        }
    }
        ...
        public void run() {
            ...
            synchronized (configFile) {
                ...
                try {
                    ...
                    if (!configFile.exists()) {
                        try {
                            LOGGER.log(Level.INFO, I18n.tl("message.config.creatingEmptyConfig", configFile.toString()));
                            if (!configFile.createNewFile()) {
                                LOGGER.log(Level.SEVERE, I18n.tl("error.config.failedToCreateConfig", configFile.toString()));
                                return;
                            }
                        } catch (IOException ex) {
                            LOGGER.log(Level.SEVERE, I18n.tl("error.config.failedToCreateConfig", configFile.toString()), ex);
                            return;
                        }
                    }
